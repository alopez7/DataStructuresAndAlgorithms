...
class ANS():
	def __init__(self, k):
		'''
		recibo como argumento un avion (k)
		'''
		self.inicio = 1
		self.pendientes = []
		self.k = k
	...
	'''Dada una ruta, ejecuta la heurística y retorna la mejor solución'''
	def Ejecutar(self, dual_pi, dual_gamma, ruta_inicial):#ruta_inicial: primera columna(ruta) de este avion que corresponde a la PB factibilizada
		'''
		Recibe una ruta inicial y ejecuta la heurística
		Los duales son para las operaciones y la función objetivo
		'''
		...
		'''
		Hago las primeras operaciones a la ruta dada (eliminar pedidos)
		'''
	 	r = self.Inicializacion(ruta_inicial)

		'''
		Preparaciones para ejecutar la rutina sucesivamente sobre la ruta
		'''
		BestSolution = self.Copiar(r)
	 	BestUtility = r.FO
		'''
		Máximo de 35 iteraciones
		'''
 		Allow = 0
 		Local_Iter = 35 #era 35 ################ DEBERIAMOS SENSIBILIZAR ESTO #####################
 		fo_nueva = 0
 		fo_inicial = BestUtility
 		tpo_rutina = 0.000000000001
		'''
		bloq indica las operaciones que se realizaron
		'''
 		bloq = []
 		for i in range(0,7):
 			bloq.append(0)
		...
		'''
		Mientras no se supere el máximo de iteraciones
		'''
 		while Allow < Local_Iter:
			'''
			Se elige una operación que no se haya hecho según su probababilidad
			se marca la operación como realizada
			se realiza la operación
			se actualiza su probabilidad segun su mejora y el tiempo que tomó
			'''
			...
			'''
			Se actualiza la ruta si se encuentra una mejor y se acutaliza la funcion objetivo
			'''
			...
			'''
			Se actualiza el contador, y si se realizaron todas las operaciones, se termina el while
			'''
			Allow = Allow + 1
		...
		'''
		No entiendo lo que hace el metodo GuardarCargas TODO
		'''
		BestSolution = self.GuardarCargas(BestSolution)
		'''
		Le asigno un id a la ruta y completo los datos que le faltan
		'''
	...
	'''Inicializa rutas para un avion dado a partir de una ruta inicial'''
	def Inicializacion(self, ruta_inicial): #Esta heuristica de Inicializacion prueba la eliminacion secuencial de los peores pedidos en la PB
		'''
		Tomo la ruta dada para el avión (r la perteneciente a la PB)
		'''
		...
		'''
		Calculo la utilidad máxima de esta ruta
		'''
		maxUtilidad = self.FuncionObjetivo(r)


		'''
		Ordeno los nodos de los pedidos segun su utilidad (tarifa - costo - costo dual)
		'''
		...
		'''
		pruebo eliminando pedidos de la ruta desde el de menos ganancia al de más ganancia
		'''
		for i in list_orden: #A continuacion se va probando la eliminacion SECUENCIAL de cada pedido
			...
			'''
			Ajusto los tiempos de la ruta
			'''
			rCopia = self.AsignarTiempos(rCopia)
			#print "RUTA CANDIDATA:" , rCopia.nodos
			'''
			Si en una ruta factible
			'''
			if rCopia.fact == 1:
				'''
				Ajusto las cargas en el vuelo
				'''
				rCopia = self.AsignarCargas(rCopia)
				'''
				Calculo la función objetivo nueva y actualizo el máximo
				'''
	...
	'''Revisa la factivilidad de una ruta y asigna los tiempos de los vuelos'''
	def AsignarTiempos(self,r): #agregar parametro de la posicion del pedido que se modifico.
		'''
		Revisa la factivilidad de una ruta y asigna los tiempos
		'''
		ruta = r.nodos
	...
	'''Asigna para cada instante del viaje las cargas correctas (Si no se lo puede, optimiza las cargas)'''
	...
	'''Optimización con guribi para cargas'''
	...
	'''Operación que elimina un pedido de una ruta'''
	def EliminarPedido(self,p,r):
		'''
		Encuentro el pickup y el devery del pedido y los elimino
		'''
	...
	'''Usado dentro de algunas operaciones'''
	...
	'''Método actualiza la probabilidad de realizar una operación según cuanto mejore y cuanto se demore'''
	...
	'''Calcula función objetivo como (Ingresos - CostosOperacioness - CostosCancelaciones - CostosDual)'''
	def FuncionObjetivo(self, r): #Funcion Objetivo del Problema Satelite/ r = Ruta()
		...
		'''
		Calculo la utilidad total y la retorno
		'''
		utilidad = Ingresos - CostosOps - CostosCanc - CostosDual

		return utilidad

	'''Calcula función objetivo como (Ingresos - CostosOperacioness - CostosCancelacion)'''
	...
	############################################################################
	# Los siguientes métodos se usan solo en la generación inicial de columnas #
	############################################################################
