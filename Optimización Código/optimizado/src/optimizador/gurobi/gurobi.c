...
void optimize_weight(Route* route)
{
  ...
  // Crep el arreglo con las constantes que multiplican las variables en la FO
  ...
  for (LNode* ln = route -> nodes -> start; ln -> next; ln = ln -> next)
  {
    ...
    // Creo la restriccion
    error = GRBaddconstr(model, 3, r4_ind, r4_val, GRB_LESS_EQUAL, 0, r_name);
    // Si da error, termino
    if (error) goto QUIT;
    // Le sumo 1 al numero de restriccion
    r_count++;
  ...
  printf("Optimizacion lista\n");
  if (optimstatus == GRB_OPTIMAL)
  {
    printf("Funcion objetivo: %.4e\n", fo_val);

    for (int i = 0; i < count; i++)
    {
      printf("ID: %d Recojo: %lf Tengo: %lf\n", i, solution[2 * i + 1], solution[2 * i]);
    }
  ...
  // Hago que sea falso que la ruta es valida
  route -> valid = false;
}
